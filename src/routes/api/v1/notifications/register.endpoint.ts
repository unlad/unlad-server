        ...
        new HTTPEndpoint({
            ...
            handlers: [
                ...
                async (server: Server, req: Request, res: Response, next: NextFunction) => {
                    ...
                    const bodyschema = z.object({ type: z.string(), token: z.string() })
                    const body = bodyschema.safeParse(req.body)
                    if (!body.success) return res.send({ code: 2 })

                    const query = await server.notifications.register(session.data.uuid, body.data.type, body.data.token)
